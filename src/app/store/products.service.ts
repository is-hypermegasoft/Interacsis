...
export class ProductsService 
{
  ...
  getAccesorios(): Observable <Product[]>{
    return this.database.collection<Product>('Products',ref=>ref.where('category','==','accesorios')).valueChanges();
  }
  getLaptops(): Observable <Product[]>{
    return this.database.collection<Product>('Products',ref=>ref.where('category','==','laptops')).valueChanges();
  }
  getPcs(): Observable <Product[]>{
    return this.database.collection<Product>('Products',ref=>ref.where('category','==','pc')).valueChanges();
  }
  getPerifericos(): Observable <Product[]>{
    return this.database.collection<Product>('Products',ref=>ref.where('category','==','perifericos')).valueChanges();
  }
  getGamers(): Observable <Product[]>{
    return this.database.collection<Product>('Products',ref=>ref.where('category','==','gamer')).valueChanges();
  }
